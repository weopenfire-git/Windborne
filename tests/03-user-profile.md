...
```
- **错误响应**：
  - 400 或 422：文件类型不支持 / 文件过大 / 文件为空 / 缺少文件字段
  - 401：未登录
...
- [ ] **TC-A301**：上传 2MB+1 字节的图片 | 预期：400 或 422，error 提示文件过大
- [ ] **TC-A302**：上传空文件（0 字节） | 预期：400 或 422，error 提示文件为空
- [ ] **TC-A303**：上传非图片文件（如 .pdf） | 预期：400 或 422，error 提示文件类型不支持
- [ ] **TC-A304**：上传 .svg 文件（不在允许列表） | 预期：400 或 422，error 提示不支持文件类型
- [ ] **TC-A305**：表单字段名不是 "avatar"（如叫 "file"） | 预期：400 或 422，error 提示缺少 avatar 字段
